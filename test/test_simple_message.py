...
class TestMotoMotionReply(unittest.TestCase):
    def test_to_and_from_bytes(self):
        moto_motion_reply = MotoMotionReply(
            groupno=1, sequence=42, command=CommandType.START_SERVOS, result=ResultType.ALARM,
            subcode=SubCode.NOT_READY_HOLD, data=[0.0] * 10
        )
        ...
        self.assertEqual(moto_motion_reply.groupno, moto_motion_reply_from_bytes.groupno)
        self.assertEqual(
            moto_motion_reply.sequence, moto_motion_reply_from_bytes.sequence
        )
        self.assertEqual(moto_motion_reply.command, moto_motion_reply_from_bytes.command)
        self.assertEqual(moto_motion_reply.result, moto_motion_reply_from_bytes.result)
        self.assertEqual(moto_motion_reply.subcode, moto_motion_reply_from_bytes.subcode)
