...
class IoConnection(SimpleMessageConnection):
    ...
    def _send_and_recv_io_request(self, msg_type: MsgType, *args: int) -> SimpleMessage:
    ...
    def read_io_bit(self, address: int):
        return self._send_and_recv_io_request(MsgType.MOTO_READ_IO_BIT, address)

    def write_io_bit(self, address: int, value: int):
        return self._send_and_recv_io_request(MsgType.MOTO_WRITE_IO_BIT, address, value)

    def read_io_group(self, address: int):
        return self._send_and_recv_io_request(MsgType.MOTO_READ_IO_GROUP, address)

    def write_io_group(self, address: int, value: int):
        return self._send_and_recv_io_request(
